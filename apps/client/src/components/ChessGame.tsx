...
export default function ChessGame() {
    ...
    useEffect(() => {
        ...
        if (game.isGameOver()) {
            ...
            if (game.isCheckmate()) {
                ...
                setWinner('draw');
                console.log('Game ended: Threefold Repetition');
            } else if (game.isInsufficientMaterial()) {
                setWinner('draw');
                console.log('Game ended: Insufficient Material');
            } else if (game.isDraw()) {
                setWinner('draw');
                console.log('Game ended: Draw (50-move rule or other)');
            } else {
    ...
    function updateGame(newGame: Chess) {
        ...
        // Actually, preventing the loss of history is better but 'new Chess(fen)' is safer for immutable state updates.
        // Let's grab the last move from the verbose history of the *new* state? No, it's empty if initialized from FEN.
        // We must calculate it BEFORE updating 'game' or catch it during the move generation.
    }
    ...
    const handleMoveSelect = (index: number) => {
        // Find safe index bound
        const safeIndex = Math.max(0, Math.min(index, moveHistory.length - 1));

        // Helper to reconstruct game state from FEN
        // We create a new chess instance from that fen
        // NOTE: chess.js history() requires playing moves, but FEN is stateless.
        // For simple replay Board Position, FEN is enough. 
        // For full history list, we will persist the 'game' object's history separately if needed,
        // but for now, we just update the board visual.
    ...
    // We can diff the board or just use internal logic if tracking history array of complete moves.
    // Ideally we'd use game.history({verbose: true}) but that resets when we load from FEN.
    // For now, simpler visual extraction from current board vs initial board is tricky without full history.
    // Let's rely on the fact that we can track material difference.
    // Actually, let's just stick to history navigation for now and add simple material count later if precise capture list is hard with FEN-only replay.
    // WAIT: We can use the history array to reconstruct the game from start to get captures?
    // Optimization: Just show what's ON the board vs mostly full set? 
    // Let's implement a simple "Material" check.

    // Helper to get captured pieces (naive diff is hard, let's skip complex capture logic for this specific step 
    // and focus on History and Replay first as they are connected).
    // Actually, let's try to pass the history of SAN moves to MoveHistory component.
    // Since we re-hydrate from FEN, we lose the SAN history in the 'game' object slightly unless we preserve it. 
    // Let's rely on 'game.history()' from the *latest* state. 
    // Problem: 'game' state changes when we click replay.
    // Solution: Keep a separate 'masterGame' reference or just storing sanctions? 
    // Let's store `moveSanHistory` string[] separately.
